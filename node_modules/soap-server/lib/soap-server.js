...
SoapServer.prototype.castArgument = function(inputName, requestArgument, options){
	// console.log('SoapServer.prototype.castArgument-->options->'+JSON.stringify(options));
	 //console.log('requestArgument-->'+JSON.stringify(requestArgument));
	switch(options.type){
		case 'object':
...
SoapServer.prototype.handleOperationRequest = function(service, soapRequest){
	...
	//console.log('operationRequest-->'+JSON.stringify(operationRequest));
	for(var operationRequestName in operationRequest){
		...
		for(var inputName in operation.inputs){
			...
			var requestArgument = this.castArgument(inputName, requestArguments['ns:'+inputName], input);
...
SoapServer.prototype.js2xml = function(js, namespace, indentLevel){
	...
	for(var node in js){
		...
		//--------------------------------添加数组判断------------------------------
		if(require('util').isArray(js[node])) {
			js[node].forEach(item => {
				ret += indent + '<' + element + '>\n';
				for (const key in item) {
					if (item.hasOwnProperty(key)) {
						const elementThis = item[key];
						ret += indent + '<' + key + '>\n';
						ret += elementThis;
						ret += indent + '</' + key + '>\n';
					}	
		...
		if('object' == typeof js[node] && !require('util').isArray(js[node])) {
			// console.log('js[node])-->'+JSON.stringify(js[node]));
			ret += indent + '<' + element + '>\n';
			ret += this.js2xml(js[node], namespace, indentLevel + 1);
			ret += indent + '</' + element + '>\n';
		}
		else if('string' == typeof js[node]) {
		...
		req.on('end', function () {
			...
			parseXml(body, function (err, soapRequest) {
				...
				try{
					...
					console.log('soapResponse-->'+ JSON.stringify(soapResponse));
					responseXml += This.js2xml(soapResponse, null, 3);
				}
				catch(exception){
...
SoapServer.prototype.getWsdl = function(host, service){
	...
	for(operationName in service.operations){
		...
		if(operation.output.type == 'object'){
			wsdl += '		<part name="parameters" element="tns:' + this.getType(operation.output) + '" />\n';
		}
		else{ //return
			wsdl += '		<part name="parameters" element="xsd:' + this.getType(operation.output) + '" />\n';
		}
		wsdl += '	</message>\n';
	...
	for(operationName in service.operations){
		var operation = service.getOperation(operationName);
		//<input wsam:Action="' + operationName + '" message="tns:' + operationName + '" />\n';
		//<output wsam:Action="http://' + this.options.tns + '/' + service.name + '/' + operationName + 'Response"\n';
		wsdl += '		<operation name="' + operationName + '">\n';
		wsdl += '			<input name="' + operationName + '" message="tns:' + operationName + '" />\n';
